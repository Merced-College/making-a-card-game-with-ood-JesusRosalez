...
public class BlackJackNEW {
  ...
  public static void main(String[] args) {
    ...
    int dealerTotal = 0; 
      ...
      if (playerDecision.equals("no")) {
        turn = false; //exit the loop to end the game
      }
    }
  ...
  private static void initializeDeck() {
    ...
    for (int i = 0; i < cards.length; i++) {
      int value = rankIndex < 9 ?
      Integer.parseInt(RANKS[rankIndex]) : 10; 
      cards[i] = new Card(value, SUITS[suitsIndex], RANKS[rankIndex]);
      suitsIndex++;
      if (suitsIndex == 4) {
        suitsIndex = 0;
        rankIndex++;
